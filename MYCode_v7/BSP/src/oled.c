...
/* 一次事务发送控制字节和最多7个显存字节，适配当前8字节TX FIFO。
 * 原实现每个像素字节都单独发起I2C事务，在100kHz下整屏刷新会长时间阻塞
 * MPU6050采样，导致转动期间的角速度积分严重丢失。 */
static void OLED_WR_DataBurst(const u8 *data, u8 len)
{
	u8 txData[8];
	u8 i;
	...
	txData[0] = 0x40U;
	for (i = 0U; i < len; i++) txData[i + 1U] = data[i];

	while (!(DL_I2C_getControllerStatus(OLED_INST) & DL_I2C_CONTROLLER_STATUS_IDLE));
	DL_I2C_fillControllerTXFIFO(OLED_INST, txData, (uint32_t)len + 1U);
	DL_I2C_startControllerTransfer(OLED_INST, 0x3C,
		DL_I2C_CONTROLLER_DIRECTION_TX, (uint32_t)len + 1U);
	while (!(DL_I2C_getControllerStatus(OLED_INST) & DL_I2C_CONTROLLER_STATUS_BUSY_BUS));
	while (!(DL_I2C_getControllerStatus(OLED_INST) & DL_I2C_CONTROLLER_STATUS_IDLE));
}

/* 只刷新指定页，便于姿态测试页降低OLED占用时间。页范围为0~7。 */
void OLED_RefreshPages(u8 first_page, u8 last_page)
{
	u8 page;

	if (first_page > 7U) return;
	if (last_page > 7U) last_page = 7U;
	...
	for (page = first_page; page <= last_page; page++)
	{
		...
		while (column < 128U)
		{
			u8 burst[7];
			u8 j;
			u8 remain = (u8)(128U - column);
			u8 chunk = (remain > 7U) ? 7U : remain;
			/* OLED_GRAM按[column][page]排列，同一页的相邻列在内存中并不连续，
			 * 因此先整理到连续小缓冲区后再批量发送。 */
			for (j = 0U; j < chunk; j++) burst[j] = OLED_GRAM[column + j][page];
			OLED_WR_DataBurst(burst, chunk);
			column = (u8)(column + chunk);
		}
	}
}
