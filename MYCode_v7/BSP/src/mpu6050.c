...
/* 参考 24_H 工程使用 ±2000dps，避免小车快速旋转时 ±250dps 饱和丢角。 */
static float s_accel_sens = MPU6050_ACCEL_SENS_2G;
static float s_gyro_sens  = MPU6050_GYRO_SENS_2000;
...
int MPU6050_Init(void)
{
    ...
    /* 陀螺仪量程: ±2000dps，快速直角转弯不会因量程饱和漏积分。 */
    MPU6050_WriteReg(MPU6050_REG_GYRO_CONFIG, MPU6050_GYRO_2000DPS);
    s_gyro_sens = MPU6050_GYRO_SENS_2000;
...
void MPU6050_Calibrate(uint16_t samples)
{
    ...
    s_gyro_z_bias_raw = (float)s_calib.gyro_z_offset;
...
void MPU6050_ReadDataCalibrated(MPU6050_Data_t *data)
{
    ...
    data->gyro_x = (float)gx / s_gyro_sens;
    data->gyro_y = (float)gy / s_gyro_sens;
    data->gyro_z = ((float)gz - s_gyro_z_bias_raw) / s_gyro_sens;

    /* 仅在模长接近 1g 且三轴角速度都很小时微调零偏。
     * 阈值刻意较小，避免把缓慢真实转动误认为温漂。 */
    {
        float acc_norm_sq = data->accel_x * data->accel_x
                          + data->accel_y * data->accel_y
                          + data->accel_z * data->accel_z;
        if (acc_norm_sq > 0.9409f && acc_norm_sq < 1.0609f &&
            fabsf(data->gyro_x) < 0.5f &&
            fabsf(data->gyro_y) < 0.5f &&
            fabsf(data->gyro_z) < 0.5f)
        {
            s_gyro_z_bias_raw += 0.001f * ((float)gz - s_gyro_z_bias_raw);
            data->gyro_z = ((float)gz - s_gyro_z_bias_raw) / s_gyro_sens;
        }
    }
...
static float s_prev_gyro_z = 0.0f;
static uint8_t s_prev_gyro_z_valid = 0U;
...
/* Yaw 单独按校准后的 gyro_z 积分；加速度计不能观测绕重力轴的绝对航向。
 * 梯形积分比单点欧拉积分更适合快速角速度变化。 */
static void MPU6050_UpdateYaw(float gyro_z, float dt)
{
    if (gyro_z > -0.25f && gyro_z < 0.25f) gyro_z = 0.0f;
    if (!s_prev_gyro_z_valid)
    {
        s_prev_gyro_z = gyro_z;
        s_prev_gyro_z_valid = 1U;
    }
    cached_yaw += 0.5f * (s_prev_gyro_z + gyro_z) * dt;
    s_prev_gyro_z = gyro_z;
    while (cached_yaw > 180.0f) cached_yaw -= 360.0f;
    while (cached_yaw < -180.0f) cached_yaw += 360.0f;
}
...
void MPU6050_QuaternionUpdateDt(float dt)
{
    ...
    MPU6050_UpdateYaw(data.gyro_z, dt);
...
void MPU6050_QuaternionUpdateFastDt(float dt)
{
    ...
    /* 减去校准零偏，Yaw 先用 deg/s 梯形积分，再转换为 rad/s 更新四元数。 */
    ...
    float gyro_z_dps = ((float)raw_gz - s_gyro_z_bias_raw) / s_gyro_sens;
    MPU6050_UpdateYaw(gyro_z_dps, dt);
    s_latest_data.gyro_x = gx * 57.29578f;
    s_latest_data.gyro_y = gy * 57.29578f;
    s_latest_data.gyro_z = gyro_z_dps;
    float gz = gyro_z_dps * 0.0174533f;
    {
        const float dz = 0.25f * 0.0174533f;
        if (gx > -dz && gx < dz) gx = 0.0f;
        if (gy > -dz && gy < dz) gy = 0.0f;
