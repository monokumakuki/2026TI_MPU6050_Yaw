...
/* 航向角 I 增益：消除电机不对称/地面倾斜造成的缓慢偏航。参考范围 0.02~0.1 */
#define YAW_KI       0.05f

/* 航向角 D 增益：抑制航向修正过冲。参考范围 1.0~3.0 */
#define YAW_KD       2.0f

/* 航向角积分限幅：防止 I 项饱和导致回正过冲。参考范围 50~150 */
#define YAW_IMAX     80.0f
 ...
 * 函 数 返 回：float 规范化后的角度误差 (target - current)，范围 -180~+180
 * 备       注：正值需右转修正，负值需左转修正
 ******************************************************************/
 ...
 * 函 数 说 明：计算航向角 PID 输出（位置式，带积分限幅）
 * 函 数 形 参：yaw_err - 规范化后的航向角误差(度)
 ...
 ******************************************************************/
static int16_t yaw_pid_compute(float yaw_err)
{
    /* P 项 */
    float p_out = YAW_KP * yaw_err;

    /* I 项：累积 + 抗饱和限幅 */
    yaw_i += yaw_err;
    if (yaw_i > YAW_IMAX)  yaw_i = YAW_IMAX;
    if (yaw_i < -YAW_IMAX) yaw_i = -YAW_IMAX;
    float i_out = YAW_KI * yaw_i;

    /* D 项：误差变化率 */
    float d_out = YAW_KD * (yaw_err - yaw_last_err);
...
void Line_Run(void)
{
    ...
    if (pivot_active)
    {
        ...
        int16_t fast = g_line_base_speed * 3 / 2;  /* base×1.5 */
        if (fast < HARD_PIVOT_SPEED) fast = HARD_PIVOT_SPEED;
        if (fast > 400) fast = 400;
    ...
    if (sensors == 0x00)
    {
        ...
        if (g_mpu_mode)
        {
            ...
            int16_t yaw_out = yaw_pid_compute(yaw_err);
    ...
    else
    {
        ...
        if (g_mpu_mode)
        {
            if (raw_error == 0)
            {
                ...
                /* 锁定后 center_cnt 不再增长（防止溢出），锁持续生效 */

                /* 航向角 PID 计算 */
                float yaw_err = yaw_error_wrap(yaw_target, mpu_yaw);
                int16_t yaw_out = yaw_pid_compute(yaw_err);

                /* 加权融合：直行时航向权重 40%，巡线权重 60%
                 * 参考网上方案：lineError*0.7 + yaw*0.3，根据实测调整 */
                pid_output = (line_pd * 3 + yaw_out * 2) / 5;  /* 60%PD + 40%YawPID */
            }
            else
            {
                /* ======================================================== */
                ...
                /* 连续陀螺阻尼：gyro_z(°/s) × 阻尼系数
                 * gyro_z > 0 = 车在右转，减去正值 → 减小右转力度（抑制甩尾）
                 * gyro_z < 0 = 车在左转，减去负值 → 减小左转力度
